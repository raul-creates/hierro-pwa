...
- `renderRoutineExercises()` (hoy lee `editRoutine.exercises`) y `buildRoutineExBlock` (hoy calcula `canRm=editRoutine.exercises.length>1`) pasan a resolver la rutina activa vía `routines.find(r=>r.id===editingRoutineId)` en vez de `editRoutine` — mismos dos puntos de lectura, misma lógica, otra fuente.
